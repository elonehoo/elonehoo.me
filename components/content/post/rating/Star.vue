...
export default {
  ...
  computed: {
    ...
    starSize() {
      ...
      const size = (this.roundedCorners && this.borderWidth <= 0) ? Number.parseInt(this.size) - Number.parseInt(this.border) : this.size
    ...
    maxSize() {
      return this.starPoints.reduce((a, b) => {
        return Math.max(a, b)
      })
    },
    viewBox() {
...
<template>
  <svg
    class="vue-star-rating-star" :class="[{ 'vue-star-rating-star-rotate': shouldAnimate }]"
    :height="starSize"
    :width="starSize"
    :viewBox="viewBox"
