...
![weird debugging tricks](/posts/0020.browser-debugging-tricks/weird-debugging-tricks.gif)
...
![console table in watch](/posts/0020.browser-debugging-tricks/console-table-in-watch.png)

或者要在 DOM 变更后执行一个表达式，可以设置一个 DOM 变更断点（在 Element Inspector 中）：

![chrome dom mutation](/posts/0020.browser-debugging-tricks/chrome-dom-mutation.png)
...
![console trace find stack](/posts/0020.browser-debugging-tricks/console-trace-find-stack.gif)
...
![debugging tricks](/posts/0020.browser-debugging-tricks/debugging-tricks.gif)
...
![console time performance profile](/posts/0020.browser-debugging-tricks/console-time-performance-profile.gif)
...
![conditional breakpoint argument length](/posts/0020.browser-debugging-tricks/conditional-breakpoint-argument-length.gif)

#### 函数参数数量不匹配的断点

仅当当前函数被调用时传入了错误的参数数量时暂停：`(arguments.callee.length) != arguments.length`

![conditional breakpoint arity check](/posts/0020.browser-debugging-tricks/conditional-breakpoint-arity-check.gif)
...
![never-pause-here](/posts/0020.browser-debugging-tricks/never-pause-here.png)

![never-pause-here-result](/posts/0020.browser-debugging-tricks/never-pause-here-result.png)
...
![conditional-breakpoint-gated](/posts/0020.browser-debugging-tricks/conditional-breakpoint-gated.png)
...
![conditional-breakpoint-gated-enable-from-breakpoint](/posts/0020.browser-debugging-tricks/conditional-breakpoint-gated-enable-from-breakpoint.png)
...
```js
const p = instance.constructor.prototype
Object.getOwnPropertyNames(p).forEach(k => monitor(p[k]))
```
...
![elusive-element](/posts/0020.browser-debugging-tricks/elusive-element.gif)
...
![elusive-element-inspected](/posts/0020.browser-debugging-tricks/elusive-element-inspected.gif)
...
![monitor-focus](/posts/0020.browser-debugging-tricks/monitor-focus.gif)

### 查找 Bold Elements

```js
function isBold(e) {
  const w = window.getComputedStyle(e).fontWeight
  return w === 'bold' || w === '700'
}
[...document.querySelectorAll('*')].filter(isBold)
```
...
```js
[...$0.querySelectorAll('*')].filter(isBold)
```
...
![getEventListeners](/posts/0020.browser-debugging-tricks/getEventListeners.png)
...
![monitorEvents](/posts/0020.browser-debugging-tricks/monitorEvents.gif)
