...
import type { Aliases, Alpha, HueOrAlias, Options, RadixHue, Step } from './types'
...
export function presetRadix<T extends Aliases>({
  ...
  return {
    ...
    shortcuts: [
      ...
      [
        /^([a-z]+(-[a-z]+)*)-([a-z]+)([1-9]|10|11|12|-fg)(A)?$/,
        (match) => {
          if (!match)
            return
          const [token, _property, _propertyInnerGroup, hueOrAlias, step, alpha = ''] = match as [
            string,
            string,
            string,
            HueOrAlias,
      ...
      [
        ...
        (match) => {
          if (!match)
            return
          const [token, alias, _aliasInnerGroup, hue] = match as [string, string, string, RadixHue]
    ...
    rules: [
      ...
      [
        /^var\(--([\w\-]+)-([a-z]+)([1-9]|10|11|12|-fg)(A)?(\)|,)?$/,
        (match, context: RuleContext<Theme>) => {
          if (!match)
            return
          const [_token, matchedPrefix, hueOrAlias, step, alpha = '', _trailingBracketOrComma] = match as [
            string,
            string,
            HueOrAlias,
            Step,
    ...
    extractors: [
      // extracts usage of css variables with radix color format.
      {
        ...
        order: 1,
        extract({ code }) {
          const mySplitRE = /var\(\s?--([\w\-]+)-([a-z]+)([1-9]|10|11|12|-fg)(A)?[\s),]/g
